...
def get_schoolkid_by_fullname(full_name):
    try:
        ...
        logging.error("Найдено несколько учеников, попробуйте указать более конкретно (Фамилия Имя) или проверьте на ошибки в написании")
        return None
...
def create_commendation(full_name, subject):
    commendations_examples = ["Молодец!", "Отлично!", "Хорошо!", "Гораздо лучше, чем я ожидал!", "Ты меня приятно удивил!", "Великолепно!", "Прекрасно!", "Ты меня очень обрадовал!", "Именно этого я давно ждал от тебя!", "Сказано здорово – просто и ясно!", "Ты, как всегда, точен!", "Очень хороший ответ!", "Талантливо!", "Ты сегодня прыгнул выше головы!", "Я поражен!",
                              "Уже существенно лучше!", "Потрясающе!", "Замечательно!", "Прекрасное начало!", "Так держать!", "Ты на верном пути!", "Здорово!", "Это как раз то, что нужно!", "Я тобой горжусь!", "С каждым разом у тебя получается всё лучше!", "Мы с тобой не зря поработали!", "Я вижу, как ты стараешься!", "Ты растешь над собой!", "Ты многое сделал, я это вижу!", "Теперь у тебя точно все получится!"]
    schoolkid = get_schoolkid_by_fullname(full_name)

    if schoolkid is None:
        return

    lesson = Lesson.objects.filter(subject__title=subject, group_letter=schoolkid.group_letter,
                                   year_of_study=schoolkid.year_of_study).order_by('-date').first()
    if lesson is None:
        logging.error("Урок {} в классе {} не найден".format(
            subject, f"{schoolkid.year_of_study}{schoolkid.group_letter}"))
        return

    Commendation.objects.create(subject=lesson.subject, teacher=lesson.teacher, created=lesson.date,
                                schoolkid=schoolkid, text=commendations_examples[random.randint(0, len(commendations_examples) - 1)])
